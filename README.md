...
- `server.py` – FastAPI service that fronts the agent with REST endpoints for events, preferences, flight searches, and reasoning logs; also exposes simple auth handlers and mock data fallbacks.
...
4. **LLM ranking** – `compute_best_flight` asks the OpenAI model to justify the top three options; failures fall back to price sorting.
5. **Short-term memory** – `InMemorySaver` and `InMemoryStore` retain node outputs across the conversation so flight candidates, parsed invitations, and calendar context persist between steps.

6. **Decision loop** – `display_flights` and `booking_or_repeat` present recommendations, collect feedback, and either confirm a booking or loop for refined input.
...
2. **Backend server**
   ...
   ```
   To make the subscriber start listening to the events
   ```bash
   python3 sub.py
   ```
   ```bash
